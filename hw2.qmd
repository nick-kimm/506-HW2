...
```
#Part c:
First need to edit the functions above to account for a set seed which we'll do by adding a another parameter value in the functions we wrote above. 
```{r}
seed=0
...
hist(log(cars_r$Highway_mpg[cars_r$Highway_mpg<100]),breaks=10,freq=TRUE)
```
...
```{r}
library(emmeans)
emmip(model2, Torque~Horsepower)
```

#Part f:
```{r}
design_mat <- data.frame(cars_r[,1:3],cars_r[,16],cars_r[,18:19])
