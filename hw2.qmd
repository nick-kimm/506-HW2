...
```{r}
#' Version 3 Dice Game
#'First it'll check if the number of rolls is 1 as this will break the indexing solution I have for the table. If it is the case x=1 then it'll check if the roll is either a 3 or a 5 and return 4 if that's true, -2 otherwise. In the case x>1 then it'll create a new column that will strictly count the number of times 3 or a 5 were rolled. Then we will sum the values in the first column and multiply that by 2 and subtract that value from the values in column 2 which we multiplied by 4 to get our total winnings.
#' @param x 
#'
#' @return Total winnings 
#'
#' @examples rolls <- 2 4 5 6 3 1
#' roll_t <- rolls 0 1
#'             1   1 0
#'             2   1 0
#'             3   0 1
...
play_dice3 <- function(x){
  rolls <- sample(1:6,x,replace=TRUE)
  ifelse(x==1&(rolls==3|rolls==5),return(4),return(-2))
...
rolls <- sample(1:6,x,replace=TRUE)
roll_t=table(rolls)
test = table(rolls,ifelse((rolls==3|rolls==5),1,0))
...
#Version 1 test:
print(play_dice(3))
print(play_dice(3000))

#Version 2 test:
print(play_dice2(3))
print(play_dice2(3000))

#Version 3 test:
print(play_dice3(3))
print(play_dice3(3000))

#Version 4 test:
print(play_dice4(3))
print(play_dice4(3000))
```
#Part c:
```{r}
