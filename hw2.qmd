...
#Part a: 
```{r}
data <- read.csv("/Users/nicholaskim/Documents/STAT 506/HW2/cars.csv")

names(data) <- c("Height","Length","Width","Dive_type","Engine_type","Is_hybrid","Num_fwd_gears","Trans_type","City_mpg","Fuel_type","Highway_mpg","Manual_or_auto","Car_id","Car_make","Model_yr","Horsepower","Torque")
```

#Part b:
```{r}
data_r <- data[data[,10]=="Gasoline",]
```

#Part c:
```{r}
plot(data_r[,11])
plot(data_r[,11],ylim=c(0,50))
```
