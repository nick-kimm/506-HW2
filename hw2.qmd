...
```{r}
plot(cars_r[,11])
plot(cars_r[,11],ylim=c(0,50))
```
From the plot of Highway MPG we can see that there is one outlier point but when looking at the rest of the distribution it looks to fall between a nice range of values and follow some linear trend. For this reason I feel we don't need a transformation of this column. 

#Part d: 
```{r}
summary(lm(Highway_mpg~Torque+Horsepower+Height+Length+Width+Model_yr,data=cars_r))
```

From the summary output we can see that Torque and Highway MPG are statistically significant. Specifically, the estimated coefficient for Torque is close to 0 or -5.102e-02. This means that while keeping all other variables constant then two individuals who differ in Torque by 1 unit are expected to be 5.102e-02 smaller. 
...
#Part f:
```{r}
#design_mat <- matrix(c(rep(1,nrow(cars_r)),
                           #cars_r[,16:18],
                           #cars_r[,1:3]),nrow = nrow(cars_r),ncol = 7)
#design_mat

test<-c(list(rep(1,nrow(cars_r))),cars_r[,16:18],cars_r[,1:3])
```
